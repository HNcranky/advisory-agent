...
**Mục đích:** Dựng nền tảng lưu trữ tài liệu unstructured để Phase 3 ingest vào và Phase 4 query được.

**Schema DB mới:**

```sql
raw_documents (
  id, school, document_type, source_url,
  raw_text, fetched_at
)
...
document_chunks (
  id, raw_document_id,
  school, program, year,
  document_type,   -- tuition_page | curriculum_pdf | faq | handbook | scholarship_policy
  topic,           -- tuition | curriculum | scholarship | dormitory | career | ...
...
Index: `(school, topic)` cho metadata filter; vector index cho similarity search.

**Repository operations:** `upsert_chunk`, `search_by_metadata`, `vector_search`

**Acceptance Criteria:**
- [ ] Migration tạo 2 bảng idempotent, tích hợp vào pipeline migration hiện có
- [ ] Index trên `(school, topic)` tồn tại sau migration
- [ ] Vector index tồn tại và hoạt động được với embedding dimension của provider hiện tại
- [ ] `ChunkRepository` có đủ 3 operations với unit test (in-memory/test DB fixture)
- [ ] Không có thay đổi nào ảnh hưởng đến structured admission data hiện tại
